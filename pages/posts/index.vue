...
<template>
  ...
  <section class="max-w-3xl mx-auto px-4 py-8 md:py-14">
    <ul v-if="posts && posts.length" class="space-y-10 list-none ml-0">
      <li
        v-for="post in posts.filter((p) => { 
          return !p.draft && p.published;
        })"
        :key="post._path"
        class="group border-b border-zinc-200 dark:border-zinc-700 pb-10 last:border-b-0"
      >
        <NuxtLink
          :to="post._path"
